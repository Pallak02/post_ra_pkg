...
def parse_dsid_list(s: Any) -> List[int]:
    if s is None: return []
    if isinstance(s, (list, tuple)): seq = s
    else:
        ss = str(s).strip()
        if ss.startswith("[") and ss.endswith("]"):
            try: seq = json.loads(ss)
            except Exception:
                ss = ss.strip("[]"); seq = [p.strip() for p in ss.split(",") if p.strip()]
        else:
            seq = [ss]
    out: List[int] = []
    for v in seq:
        try: out.append(int(str(v).strip()))
...
def try_parse_vector(s: Any) -> Optional[List[float]]:
    ...
    ss = str(s).strip()
    if not (ss.startswith("[") and ss.endswith("]")): return None
    try:
        arr = json.loads(ss); return [float(v) for v in arr]
    except Exception:
        try:
            ss = ss.strip("[]"); parts = [p for p in ss.split(",") if p.strip()]
...
def vector_to_scalar(vec: List[float], policy: str, index: int) -> Optional[float]:
    ...
    if policy == "max": return max(vec)
    if policy == "take": return vec[index] if 0 <= index < len(vec) else None
    if policy == "argmax": return float(max(range(len(vec)), key=lambda i: vec[i]))
    if policy == "mean": return sum(vec) / len(vec)
    return None
...
def nearest_row_by_time(rows: List[Dict[str, Any]], t: float, tol: float) -> Optional[Dict[str, Any]]:
    ...
    for r in rows:
        ...
        try: dt = abs(float(ts) - float(t))
        except Exception: continue
        if dt <= tol and (best_dt is None or dt < best_dt):
            best = r; best_dt = dt
    return best

def pick_x_seconds_cols(rows: List[Dict[str, Any]], prefer_col: str = "") -> None:
    """ Mutates rows per casualty, setting row['_t'] to seconds since start. """
    by_cas: Dict[int, List[Dict[str, Any]]] = defaultdict(list)
    for r in rows:
        try: cas = int((r.get("casualty") or r.get("casualty_id") or 0))
        except Exception: cas = 0
        by_cas[cas].append(r)

    for cas, cas_rows in by_cas.items():
        choice = None
        candidates = ([prefer_col] if prefer_col else []) + [
            "time_since_start_s", "time_since_start", "time",
            "timestamp_sec", "timestamp_ns"
        ]
        for col in candidates:
            if not col: continue
            vals = [float(r[col]) for r in cas_rows if col in r and is_number(r[col])]
            if vals: choice = col; break

        if choice is None:
            for r in cas_rows: r["_t"] = math.nan
            continue

        def to_seconds(v, col):
            if not is_number(v): return math.nan
            vv = float(v)
            return vv/1e9 if col == "timestamp_ns" else vv

        times = [to_seconds(r.get(choice), choice) for r in cas_rows]
        finite = [t for t in times if not math.isnan(t)]
        offset = min(finite) if finite else 0.0
        norm = [(t - offset) if not math.isnan(t) else math.nan for t in times]
        for r, t in zip(cas_rows, norm): r["_t"] = t

# ---------------- column filtering ----------------
# Hide noisy sub-fields: stamp/*.sec, *.nanosec, *_num_observations, headers, time columns, ids, etc.
BAD_COL_REGEX = re.compile(
    r"""(?ix)
    (?:^|_)seq$|
    ^topic$|
    ^casualty_id$|
    ^data_source_ids?$|
    ^(?:time|time_since_start(?:_s)?)$|
    ^timestamp_(?:ns|sec)$|
    (?:^|_)(?:header|status)(?:$|[._])|
    (?:^|_)stamp(?:$|_(?:sec|nanosec)$)|
    (?:^|_)\w*stamp\.(?:sec|nanosec)$|
    (?:^|_)position_covariance(?:_type)?$|
    (?:^|_)\w*_num_observations$|
    ^times_submitted$|
    ^golden_window_period$|
    ^frame_id$
    """
)

def looks_like_value_col(name: str) -> bool:
    return not BAD_COL_REGEX.search(name)
...
def load_assessments_csv(csv_path: Path, mode: str = "auto"):
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as f:
        ...
        for raw in rd:
            ...
            if "casualty" not in r and "casualty_id" not in r:
                raise ValueError("CSV must include 'casualty' or 'casualty_id'.")
            rows.append(r)

    # compute _t
    pick_x_seconds_cols(rows)

    # group by casualty
    by_cas: Dict[int, List[Dict[str, Any]]] = defaultdict(list)
    for r in rows:
        try: by_cas[int((r.get("casualty") or r.get("casualty_id") or 0))].append(r)
        except Exception: by_cas[0].append(r)
    ...
    for cas, cas_rows in by_cas.items():
        ...
        for k in sample_keys:
            ...
            if mode == "all":
                candidates.append(k); continue
            # mode == auto → require variability/numeric
            vals: List[float] = []
            for r in cas_rows[:1000]:
                ...
                if is_number(v):
                    ...
                    if vec is not None: vals.append(sum(vec))
        ...
        fields_by_cas[cas] = (preferred + others)[:200]
...
ODS_TIME_KEYS   = ("time_since_start", "time_since_start_s", "t", "time", "timestamp_sec", "timestamp_ns")
...
def load_ods_json(ods_path: Path, bagb_offset_s: float, force_ods_casualty: Optional[int]):
    ...
    for r in ods_rows:
        ...
        for tk in ODS_TIME_KEYS:
            if tk in r and is_number(r[tk]):
                val = float(r[tk]); 
                if tk == "timestamp_ns": val /= 1e9
                t = val; break
        if t is not None and bagb_offset_s: t += float(bagb_offset_s)
        r["time_since_start"] = t

        cas = int(force_ods_casualty) if force_ods_casualty is not None else (_coerce_int(r.get("casualty")) or 0)
        r["casualty"] = cas; ods_by_cas[cas].append(r)
        ...
        if dsid is not None: dsid_index[dsid] = r

    # normalize per-casualty start
    for cas, rows in ods_by_cas.items():
        ts = [r["time_since_start"] for r in rows if r["time_since_start"] is not None]
        if ts:
            off = min(ts)
            for r in rows:
                if r["time_since_start"] is not None:
                    r["time_since_start"] -= off
    return ods_rows, ods_by_cas, dsid_index

# ---------- media path ----------
def resolve_media_path(candidate: str, page_dir: Path, image_path_root: Optional[str], indir: Path) -> Tuple[Optional[str], Optional[str]]:
    if not candidate: return None, "empty image path"
    s = str(candidate)
    if s.startswith(("http://","https://","file://")): return s, None
    if s.startswith("/"):
        p = Path(s)
        if p.exists():
            try: return os.path.relpath(p, page_dir), None
            except Exception: return s, None
        return None, f"not found: {s}"
    roots: List[Path] = []
    if image_path_root: roots.append(Path(image_path_root))
    roots += [indir, page_dir]
    for root in roots:
        p = root / s
        if p.exists():
            try: return os.path.relpath(p, page_dir), None
...
def quantile_edges(sorted_vals: List[float], q: int) -> List[float]:
    ...
    for k in range(1,q):
        pos=k*(n-1)/q; lo=int(math.floor(pos)); hi=int(math.ceil(pos))
        v = sorted_vals[lo] if lo==hi else (sorted_vals[lo] + (sorted_vals[hi]-sorted_vals[lo])*(pos-lo))
...
def assign_tertile_label(t: float, edges: List[float]) -> str:
    ...
    return "stage 3 (late)"
def assign_quantile_label(idx: int, q: int) -> str: return f"stage {idx+1}"
...
def build_field_page(outdir: Path, cas: int, field: str, points: List[Dict[str, Any]],
    ...
    if stage_mode == "by_field" and stage_field:
        ...
        times = sorted([p["t"] for p in points]); edges=quantile_edges(times,q)
        if stage_mode=="tertiles":
            for p in points: stage_groups.setdefault(assign_tertile_label(p["t"],edges),[]).append(p)
        else:
            for p in points:
                idx=0
                while idx<len(edges) and p["t"]>edges[idx]: idx+=1
                stage_groups.setdefault(assign_quantile_label(idx,q),[]).append(p)
    else:
        stage_groups["all"]=points

    traces=[]; total_dsid=total_time=total_tx=total_audio=total_img=0
    tx_x=tx_y=tx_cd=[] , [] , []
    au_x=au_y=au_cd=[] , [] , []
    im_x=im_y=im_cd=[] , [] , []

    for stage_label, pts in stage_groups.items():
        xs=[]; ys=[]; custom=[]; c_dsid=c_time=c_tx=c_audio=c_img=0
        for p in pts:
            t,y,last_dsid=p["t"],p["y"],p.get("last_dsid"); ods=None
            if link_mode=="auto":
                if isinstance(last_dsid,int) and last_dsid in dsid_index:
                    ods=dsid_index[last_dsid]; c_dsid+=1
                if ods is None and ods_rows_for_cas:
                ...
                for k in AUDIO_KEYS:
                    if ods.get(k):
                        ...
                        if image_path_root and not au.startswith(("http://","https://","file:/","/")):
                            abs_path=Path(image_path_root)/au; audio_url=os.path.relpath(abs_path,outdir)
                        else:
            ...
            if transcript: c_tx+=1; tx_x.append(t); tx_y.append(y); tx_cd.append(cd)
            if audio_url: c_audio+=1; au_x.append(t); au_y.append(y); au_cd.append(cd)
            if image_url: c_img+=1; im_x.append(t); im_y.append(y); im_cd.append(cd)

        total_dsid+=c_dsid; total_time+=c_time; total_tx+=c_tx; total_audio+=c_audio; total_img+=c_img
        traces.append({
    ...
    overlay_traces=[]
    if tx_x: overlay_traces.append({"x":tx_x,"y":tx_y,"type":"scatter","mode":"markers","name":"has transcript","marker":{"size":11,"symbol":"square-open","line":{"width":1.5}},"hoverinfo":"skip","showlegend":True,"customdata":tx_cd})
    if au_x: overlay_traces.append({"x":au_x,"y":au_y,"type":"scatter","mode":"markers","name":"has audio","marker":{"size":11,"symbol":"diamond-open","line":{"width":1.5}},"hoverinfo":"skip","showlegend":True,"customdata":au_cd})
    if im_x: overlay_traces.append({"x":im_x,"y":im_y,"type":"scatter","mode":"markers","name":"has image","marker":{"size":11,"symbol":"x"},"hoverinfo":"skip","showlegend":True,"customdata":im_cd})
    ...
    banner_html = f"<div id='banner'>{banner}</div>" if show_banner and banner else ""
    img_if = """
      if (imageUrl) {
...
def main():
    ap = argparse.ArgumentParser(
        description="Plot assessments and optionally link ODS media/transcripts.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    # ODS optional
    ap.add_argument("--indir", default=".", help="Folder that may contain observation_data_sources.json")
    ap.add_argument("--allow_no_ods", action="store_true",
                    help="Proceed without ODS; generate assessment-only plots if ODS is absent")

    ap.add_argument("--assess_csv", required=True, help="Assessments CSV")
    ap.add_argument("--outdir", required=True, help="Output folder for HTML plots")
    ap.add_argument("--fields", nargs="*", default=[], help="Specific fields to plot (default: auto/all per mode)")
    ap.add_argument("--fields_mode", choices=["auto","all"], default="auto",
                    help="auto: smart subset; all: every value field after filtering sub-fields")
    ...
    ap.add_argument("--enable_images", action="store_true", help="Render image if available")
    ap.add_argument("--image_field", default="image_url", help="ODS image field name")
    ap.add_argument("--image_path_root", default="", help="Root for relative media paths")

    # vector handling & staging
    ap.add_argument("--vector_policy", choices=["max","take","argmax","mean"], default="max")
    ap.add_argument("--vector_index", type=int, default=1)
    ap.add_argument("--stage_mode", choices=["none","tertiles","quantiles","by_field","vector"], default="none")
    ap.add_argument("--stage_quantiles", type=int, default=3)
    ap.add_argument("--stage_field", default="")
    ap.add_argument("--stage_labels", action="append", default=[])

    # x-axis & banner control
    ap.add_argument("--xcol", default="", help="Preferred time column to use for x (seconds).")
    ap.add_argument("--show_banner", action="store_true", help="Show the meta banner (hidden by default).")
    ...
    for entry in args.stage_labels:
        if "=" in entry:
            ...
            if field.strip(): label_map[field.strip()] = labels

    indir = Path(args.indir); outdir = Path(args.outdir); ensure_dir(outdir)
    ods_path = indir / "observation_data_sources.json"

    # Assessments
    assess_rows, fields_by_cas = load_assessments_csv(Path(args.assess_csv), mode=args.fields_mode)
    pick_x_seconds_cols(assess_rows, prefer_col=args.xcol)

    # ODS (optional)
    ods_present = ods_path.exists()
    if ods_present:
        ods_rows, ods_by_cas, dsid_index = load_ods_json(ods_path, args.bagb_offset_s, args.force_ods_casualty)
    else:
        if not args.allow_no_ods:
            raise FileNotFoundError(f"Missing file: {ods_path} (pass --allow_no_ods to plot assessments without ODS)")
        ods_rows, ods_by_cas, dsid_index = [], defaultdict(list), {}

    # choose fields per casualty
    if args.fields:
        csv_keys = set(assess_rows[0].keys())
        cas_to_fields = {cas: [f for f in args.fields if f in csv_keys] or auto
                         for cas, auto in fields_by_cas.items()}
    else:
        cas_to_fields = fields_by_cas
    ...
    print(f"[diag] ODS present: {ods_present} | Total ODS rows: {len(ods_rows)} | DSID index size: {len(dsid_index)}")
    ...
    for cas in casualties:
        ...
        cas_rows = [r for r in assess_rows if int((r.get("casualty") or r.get("casualty_id") or 0)) == cas and is_number(r.get("_t"))]
        ...
        for field in fields:
            ...
            if ods_present:
                ...
                banner = (f"vector_policy={args.vector_policy}"
                          f"{'('+str(args.vector_index)+')' if args.vector_policy=='take' else ''}")
            ...
            by_dsid, by_time, tx, au, im, page = build_field_page(
                ...
                stage_quantiles=args.stage_quantiles, indir=indir, show_banner=args.show_banner
            )
